...
//                broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
    ...
    public void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.i("log", "BluetoothAdapter not initialized");
            return;
        }
        mBluetoothGatt.readCharacteristic(characteristic);
        byte[] charValue = characteristic.getValue();
        if(charValue != null)
            Log.i("log", "readCharacteristic BluetoothChar "+ charValue.toString());
        else {
            Log.i("log", "readCharacteristic BluetoothChar not received");
        }
//        characteristic.getValue();
    ...
    private void broadcastUpdate(final String action,
        ...
        Logger.d(" val", "byte :" + data[0] +" "+data[1]);
        ...
        if(action.equals(ACTION_SMOKE_DATA_AVAILABLE)) {
            ...
            if (data != null && data.length > 0) {
                ...
                intent.putExtra(SMOKE_DATA, String.valueOf(smokeVal));
                sendBroadcast(intent);
            }
        }
        else if (action.equals(ACTION_BULB_DATA_AVAILABLE)) {
            final Intent intent1 = new Intent(action);
            Logger.d("bulb detector val", "byte :" + data[0]);
            intent1.putExtra(BULB_DATA, String.valueOf(data[0]));
    ...
    public boolean initialize() {
        ...
        if (mBluetoothManager == null) {
            ...
            if (mBluetoothManager == null) {
                Logger.i("log", "Unable to initialize BluetoothManager.");
                return false;
            }
        }
        ...
        if (mBluetoothAdapter == null) {
            Logger.i("log", "Unable to obtain a BluetoothAdapter.");
            return false;
        }
    ...
    public boolean connect(final String address) {
        if (mBluetoothAdapter == null || address == null) {
            Logger.i("log", "BluetoothAdapter not initialized or unspecified address.");
            return false;
        }
        ...
        if (mBluetoothDeviceAddress != null && address.equals(mBluetoothDeviceAddress)
                && mBluetoothGatt != null) {
            Logger.i("log", "Trying to use an existing mBluetoothGatt for connection.");
        ...
        if (device == null) {
            Logger.i("log", "Device not found.  Unable to connect.");
            return false;
        }
        // We want to directly connect to the device, so we are setting the autoConnect
        // parameter to false.
        mBluetoothGatt = device.connectGatt(this, false, mGattCallback);
        Logger.i("log", "Trying to create a new connection.");
